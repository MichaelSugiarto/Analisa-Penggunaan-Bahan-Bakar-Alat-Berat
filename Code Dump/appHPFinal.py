...
def load_data():
    ...
    # 0. Load Master Data Membaca file Master ber-HP
    try:
        df_master = pd.read_excel('cost & bbm 2022 sd 2025 HP.xlsx', sheet_name='Sheet2', header=1)
    ...
    # 1. Load Data Unit Aktif & Inaktif
    possible_files = ['Benchmark_Per_HP_Alat_Berat.xlsx']
    for f in possible_files:
        try:
            data_unit = pd.read_excel(f, sheet_name='Unit_Aktif')
...
# ==============================================================================
if df_unit is None:
    st.error("⚠️ File Benchmark_Per_HP_Alat_Berat.xlsx tidak ditemukan.")
    st.stop()

# --- [UPDATE] FUNGSI PENAMBAHAN SATUAN CAPACITY (TON / FEET) ---
...
if search_unit:
    ...
    if not res_all.empty:
        ...
        # [UPDATE] Terapkan Satuan Ton/Feet pada kolom Capacity
...
if not df_inactive_show.empty:
    with st.expander(f"⚠️ {len(df_inactive_show)} Unit Tidak Masuk Analisa"):
        # [UPDATE] Terapkan Satuan Ton/Feet untuk tabel unit inaktif
...
with tab_a:
    ...
    # [UPDATE] Urutkan dari Fuel Ratio Terboros ke Terefisien
    df_display_active.sort_values(by='Fuel_Ratio', ascending=False, inplace=True)
    
    # [UPDATE] Terapkan Satuan Ton/Feet untuk tabel Unit Aktif
    ...
    # [UPDATE] 'Capacity' dihilangkan dari style.format karena datanya sudah berbentuk string
    st.dataframe(
